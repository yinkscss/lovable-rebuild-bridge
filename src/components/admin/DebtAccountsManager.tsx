...
const DebtAccountsManager: React.FC<DebtAccountsManagerProps> = ({ applicationId, readonly = false }) => {
  ...
  const saveEdit = async () => {
    if (!editingId || !editForm || readonly) return;
  ...
  const deleteAccount = async (id: string) => {
    if (readonly || !confirm('Are you sure you want to delete this debt account?')) return;
  ...
  return (
    <div className={`${readonly ? '' : 'bg-white rounded-lg border border-gray-200'}`}>
      {!readonly && (
      ...
      {debtAccounts.length === 0 ? (
        ...
        <div className={readonly ? 'space-y-4' : 'divide-y divide-gray-200'}>
          {debtAccounts.map((account) => (
            <div key={account.id} className={readonly ? 'bg-gray-50 rounded-lg p-4' : 'p-6'}>
              {editingId === account.id && !readonly ? (
                // Edit Mode
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h4 className={`text-lg font-medium ${readonly ? 'text-blue-900' : 'text-gray-900'}`}>
                        {account.original_creditor}
                      </h4>
                      <p className={`text-sm ${readonly ? 'text-blue-700' : 'text-gray-600'}`}>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      <span className={`font-medium ${readonly ? 'text-blue-700' : 'text-gray-700'}`}>
                        Current Balance:
                      </span>
                      <p className={readonly ? 'text-blue-900 font-semibold' : 'text-gray-900'}>
                    ...
                    <div>
                      ...
                      <p className={readonly ? 'text-blue-900' : 'text-gray-900'}>
                    ...
                    <div className="mt-3 flex space-x-4">
                      {account.account_sold && (
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          readonly ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800'
                        }`}>
                          Account Sold
                        </span>
                      )}
                      {account.paid_off && (
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          readonly ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                        }`}>
                          Paid Off
                        </span>
                      )}
